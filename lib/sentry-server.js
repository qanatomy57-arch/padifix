...
async function captureServerException(err, context = {}) {
  ...
  const stacktrace = err.stack ? parseServerStackTrace(err.stack, context.route) : undefined;
...
/**
 * Parse Node.js error.stack into Sentry-compliant frames
 */
function parseServerStackTrace(stackStr, routeFallback = 'api') {
  ...
  for (const line of lines) {
    ...
    let filename = locMatch ? locMatch[1] : locationPart;
    const lineno = locMatch ? parseInt(locMatch[2], 10) : undefined;
    const colno = locMatch && locMatch[3] ? parseInt(locMatch[3], 10) : undefined;

    if (filename === 'sentry-server.js' || filename.includes('node:internal')) {
      continue;
    }

    frames.push({
      filename: filename || routeFallback || 'api.js',
      function: funcName,
      lineno,
      colno,
      in_app: !locationPart.includes('node_modules')
    });
  }

  if (frames.length === 0) {
    return { frames: [{ filename: routeFallback || 'api', function: 'handler' }] };
  }
...
/**
 * Universal Response Polyfill
 * Guarantees res.status(), res.send(), res.json() exist even if incoming res is raw Node http.ServerResponse
 */
function polyfillResponse(res) {
  ...
  if (typeof res.send !== 'function') {
    res.send = function (body) {
      if (!res.headersSent && typeof body === 'string' && !res.getHeader('Content-Type')) {
