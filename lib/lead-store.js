...
const { generateReviewToken } = require('./review-token');
...
function seedInitialLeads() {
  ...
  const initialSeeds = [
    ...
    },
    {
      ...
      review_token: null,
      review_requested_at: null,
      invoice_ref: 'INV-PF-DB8500',
      invoice_data: {
        invoice_number: 'INV-PF-DB8500',
        job_ref: 'PF-101_03',
        invoice_type: 'quote',
        status: 'issued',
        provider: {
        ...
        items: {
          workmanship: [
            { description: 'Main distribution board overhaul & safety rewiring', quantity: 1, unit_price_kobo: 3500000, amount_kobo: 3500000 }
          ],
          materials: [
            { description: '63A Double Pole Isolator & Enclosure', quantity: 1, unit_price_kobo: 2000000, amount_kobo: 2000000 },
            { description: 'Surge Protection Device (SPD 40kA)', quantity: 1, unit_price_kobo: 3000000, amount_kobo: 3000000 }
          ]
        },
        workmanship_subtotal_kobo: 3500000,
        materials_subtotal_kobo: 5000000,
        subtotal_kobo: 8500000,
        discount_kobo: 0,
        total_kobo: 8500000,
        currency: 'NGN',
        bank_details: {
          bank_name: 'GTBank',
          account_name: 'Babatunde Electric & Solar Ltd',
          account_number: '0123456789'
        },
        terms: '50% commitment deposit before commencement, balance upon full satisfaction. 30-day workmanship guarantee.',
        created_at: new Date(now - 24 * 3600 * 1000).toISOString(),
        updated_at: new Date(now - 24 * 3600 * 1000).toISOString()
      },
      created_at: new Date(now - 24 * 3600 * 1000).toISOString()
    },
    {
...
const LeadStore = {
      ...
      if (rec) {
        if (!status || rec.status === status) {
          allLeads.push({
            ...
            invoice_data: rec.invoice_data || null,
  ...
  updateLead(providerId, leadId, updates = {}) {
    ...
    return {
      ...
      lead: {
        ...
        invoice_ref: lead.invoice_ref || null,
        invoice_data: lead.invoice_data || null,
  ...
  recordReviewRequested(providerId, leadId) {
    ...
    // HMAC-SHA256 signed review invitation token (reuse existing if present)
    if (!lead.review_token) {
      lead.review_token = generateReviewToken({ leadId: lead.id, providerId: provId });
    }
  ...
  /**
   * Phase 033: Save or update itemized digital quote / invoice for a lead
   */
  saveLeadInvoice(providerId, leadId, invoicePayload = {}) {
    const provId = Number(providerId);
    const lead = leadRecords.get(leadId);
    ...
    if (lead.provider_id !== provId) {
      return { error: 'Forbidden: You do not have permission to generate or update an invoice for this lead.', statusCode: 403 };
    }
    ...
    const normType = ['quote', 'invoice'].includes(String(invoice_type).toLowerCase()) ? String(invoice_type).toLowerCase() : 'quote';

    // Parse dual-category items
    const rawWorkmanship = Array.isArray(items.workmanship) ? items.workmanship : [];
    const rawMaterials = Array.isArray(items.materials) ? items.materials : [];

    if (rawWorkmanship.length === 0 && rawMaterials.length === 0) {
      return { error: 'At least one workmanship or materials line item is required.', statusCode: 400 };
    }

    const cleanWorkmanship = [];
    let workmanshipSubtotal = 0;

    for (const item of rawWorkmanship) {
      if (!item || typeof item !== 'object') continue;
      const desc = sanitizeNotes(String(item.description || 'Labor / Workmanship'));
      if (!desc) {
        return { error: 'Workmanship item description cannot be empty.', statusCode: 400 };
      }
      if (desc.length > 200) {
        return { error: 'Workmanship item description cannot exceed 200 characters.', statusCode: 400 };
      }
      const qty = Number(item.quantity !== undefined ? item.quantity : 1);
      if (isNaN(qty) || qty <= 0 || !Number.isInteger(qty) || qty > 1000) {
        return { error: 'Workmanship item quantity must be a positive integer between 1 and 1000.', statusCode: 400 };
      }
      const unitPrice = Number(item.unit_price_kobo !== undefined ? item.unit_price_kobo : (item.amount_kobo || 0));
      if (isNaN(unitPrice) || unitPrice < 0 || !Number.isInteger(unitPrice) || unitPrice > 1000000000) {
        return { error: 'Workmanship unit price must be a non-negative integer kobo.', statusCode: 400 };
      }
      const itemTotal = qty * unitPrice;
      workmanshipSubtotal += itemTotal;
      cleanWorkmanship.push({
        description: desc,
        quantity: qty,
        unit_price_kobo: unitPrice,
        amount_kobo: itemTotal
      });
    }
    ...
    for (const item of rawMaterials) {
      if (!item || typeof item !== 'object') continue;
      const desc = sanitizeNotes(String(item.description || 'Part / Material'));
      if (!desc) {
        return { error: 'Material item description cannot be empty.', statusCode: 400 };
      }
      if (desc.length > 200) {
        return { error: 'Material item description cannot exceed 200 characters.', statusCode: 400 };
      }
      const qty = Number(item.quantity !== undefined ? item.quantity : 1);
      if (isNaN(qty) || qty <= 0 || !Number.isInteger(qty) || qty > 1000) {
        return { error: 'Material item quantity must be a positive integer between 1 and 1000.', statusCode: 400 };
      }
      const unitPrice = Number(item.unit_price_kobo !== undefined ? item.unit_price_kobo : (item.amount_kobo || 0));
      if (isNaN(unitPrice) || unitPrice < 0 || !Number.isInteger(unitPrice) || unitPrice > 1000000000) {
        return { error: 'Material unit price must be a non-negative integer kobo.', statusCode: 400 };
      }
      const itemTotal = qty * unitPrice;
    ...
    const subtotalKobo = workmanshipSubtotal + materialsSubtotal;

    let discountKobo = 0;
    if (discount_kobo !== undefined && discount_kobo !== null) {
      discountKobo = Number(discount_kobo);
      if (isNaN(discountKobo) || discountKobo < 0 || !Number.isInteger(discountKobo)) {
        return { error: 'discount_kobo must be a non-negative integer.', statusCode: 400 };
      }
      if (discountKobo > subtotalKobo) {
        return { error: `Discount (${discountKobo} kobo) cannot exceed subtotal (${subtotalKobo} kobo).`, statusCode: 400 };
      }
    }

    const totalKobo = subtotalKobo - discountKobo;

    // Stable server-authoritative invoice numbering: INV-PF-XXXXXXXX (8 uppercase hex characters)
    let invoiceNumber = lead.invoice_ref;
    if (!invoiceNumber) {
      // Collision-safe loop against active store
      let candidate = '';
      let attempts = 0;
      do {
        candidate = `INV-PF-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
        attempts++;
      } while (Array.from(leadRecords.values()).some(l => l.invoice_ref === candidate) && attempts < 10);
      invoiceNumber = candidate;
    }

    const jobRef = `PF-${lead.id.replace(/[^A-Za-z0-9]/g, '').slice(-6).toUpperCase()}`;

    // Clean direct bank transfer details (artisan's own payout details)
    const cleanBank = {
      bank_name: sanitizeText(bank_details.bank_name || '', 60) || '',
      account_name: sanitizeText(bank_details.account_name || '', 100) || '',
      account_number: sanitizeText(bank_details.account_number || '', 20) || ''
    };

    const cleanTerms = sanitizeText(terms || '50% commitment deposit before commencement, balance upon full satisfaction. 30-day workmanship guarantee.', 500);

    const isDraft = status === 'draft';
    const effectiveInvoiceStatus = isDraft ? 'draft' : (status === 'paid' ? 'paid' : 'issued');

    const invoiceData = {
      invoice_number: invoiceNumber,
      job_ref: jobRef,
      invoice_type: normType,
      status: effectiveInvoiceStatus,
      provider: {
        business_name: sanitizeText(invoicePayload.provider?.business_name || '', 100) || 'Verified Artisan',
        trade: sanitizeText(invoicePayload.provider?.trade || lead.intent_tag || '', 60) || 'Artisan Services'
      },
      customer: {
        name: sanitizeText(invoicePayload.customer?.name || lead.client_display_name || 'Valued Customer', 100)
      },
      items: {
        workmanship: cleanWorkmanship,
        materials: cleanMaterials
      },
      workmanship_subtotal_kobo: workmanshipSubtotal,
      materials_subtotal_kobo: materialsSubtotal,
      ...
      terms: cleanTerms,
      created_at: lead.invoice_data?.created_at || new Date().toISOString(),
      updated_at: new Date().toISOString()
    };

    if (effectiveInvoiceStatus === 'paid') {
      invoiceData.paid_at = lead.invoice_data?.paid_at || new Date().toISOString();
    }

    // Synchronize canonical lead columns atomically
    lead.invoice_ref = invoiceNumber;
    lead.invoice_data = invoiceData;
    lead.workmanship_amount_kobo = workmanshipSubtotal;
    lead.materials_amount_kobo = materialsSubtotal;
    lead.quote_amount_kobo = totalKobo;

    // Separate draft vs sent semantics:
    // Draft quotes remain draft with NO lead pipeline transition.
    // Transition to quote_sent occurs only when provider explicitly sends quote/invoice.
    if (!isDraft && (lead.status === 'new' || lead.status === 'in_discussion')) {
      lead.status = 'quote_sent';
    }

    // NEVER mark completed merely because an invoice is saved or generated!
    // Financial documents are records of a proposed/claimed transaction; they are not proof of settlement.
    if (effectiveInvoiceStatus === 'paid') {
      lead.final_amount_kobo = totalKobo;
    }
  ...
  /**
   * Phase 033: Retrieve invoice document for a specific lead
   */
  getLeadInvoice(providerId, leadId) {
    const provId = Number(providerId);
    const lead = leadRecords.get(leadId);
    if (!lead) {
      return { error: 'Lead not found.', statusCode: 404 };
    ...
    if (lead.provider_id !== provId) {
      return { error: 'Forbidden: You do not have permission to view an invoice for this lead.', statusCode: 403 };
    }
    if (!lead.invoice_data) {
      return { error: 'No invoice has been created for this lead yet.', statusCode: 404 };
    }
    return {
      success: true,
      invoice_number: lead.invoice_ref || lead.invoice_data.invoice_number,
      job_ref: lead.invoice_data.job_ref,
      invoice: lead.invoice_data,
      lead: {
        id: lead.id,
        provider_id: lead.provider_id,
        channel: lead.channel,
  ...
  /**
   * Phase 033: Explicit provider action to record payment settlement against an issued invoice
   */
  markInvoicePaid(providerId, leadId) {
    const provId = Number(providerId);
    const lead = leadRecords.get(leadId);
    if (!lead) {
      return { error: 'Lead not found.', statusCode: 404 };
    ...
    if (lead.provider_id !== provId) {
      return { error: 'Forbidden: Unauthorized.', statusCode: 403 };
    }
    if (!lead.invoice_data) {
      return { error: 'No invoice exists for this lead to mark as paid.', statusCode: 400 };
    }

    lead.invoice_data.status = 'paid';
    lead.invoice_data.paid_at = new Date().toISOString();
    lead.invoice_data.updated_at = new Date().toISOString();
    lead.final_amount_kobo = lead.invoice_data.total_kobo;
    lead.updated_at = new Date().toISOString();
