 ...
 * @returns {Promise<{ allowed: boolean, current_count: number, max_cap: number, scope: string, window_date: string }>}
 */
async function reserveDailySmsQuota({ scope, date, cap, _inject }) {
  const targetDate = date || getLagosDateString();
  const targetCap = Number(cap || (scope === 'platform' ? 1000 : 10));
  ...
  if (scope.startsWith('artisan:') && _inject?.mockArtisanQuotaExceeded) {
    return { allowed: false, current_count: targetCap, max_cap: targetCap, scope, window_date: targetDate };
  }

  // Attempt atomic reservation via Supabase PostgreSQL RPC if configured
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          ...
          current_count: Number(result?.current_count || 0),
          max_cap: targetCap,
          scope,
      ...
      // Fallback seamlessly to local in-memory reservation
    }
  }

  // Concurrency-safe in-memory reservation fallback
  ...
  try {
    ...
    if (_inject?.forceHttp500) {
      ...
      await logNotificationToDb({
        ...
        termiiMessageId: null,
        messageBody: smsMessage,
        errorMessage: 'Termii server error HTTP 500 (injected)'
      });
      return {
        delivered: false,
        status: 'failed',
        httpStatus: 500,
        error: 'Termii gateway HTTP 500'
      };
    }

    // 6. Artisan Daily Notification Cap Gate (Section 9)
    ...
    // 7. Platform Daily SMS Budget Circuit Breaker (Section 5, 6, 7, 8)
    ...
    // 8. Sender ID Approved Activation Gate
