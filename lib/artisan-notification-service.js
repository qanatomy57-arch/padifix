...
async function reserveDailySmsQuota({ scope, date, cap, _inject }) {
  ...
  if (SUPABASE_URL && SUPABASE_ANON_KEY && !_inject?.forceMemoryQuota) {
    try {
      ...
      // Phase 026 Section 8: Handle Supabase RPC 404 (function not installed in DB)
      // Fall back to in-memory daily quota ledger rather than falsely treating as exhausted
      if (res.status === 404) {
        return reserveMemoryDailySmsQuota({ scope, targetDate, targetCap, rpcFallback: true });
      }
  ...
  // Helper for in-memory daily quota enforcement
  function reserveMemoryDailySmsQuota({ scope, targetDate, targetCap, rpcFallback = false }) {
    ...
    if (record.count < targetCap) {
      ...
      return {
        ...
        scope,
        window_date: targetDate,
        fallback: rpcFallback ? 'rpc_404_memory_fallback' : 'memory'
      };
    } else {
      return {
        allowed: false,
...
// RFC 4122 UUID v4 regex for PostgreSQL type-safety
const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * Log notification attempt authoritatively into public.artisan_notifications
 * Validates contactEventId to guarantee it conforms to UUID before insertion into PostgreSQL.
 * If invalid or non-UUID, safely passes null to prevent PostgreSQL syntax/constraint errors.
 */
async function logNotificationToDb({ contactEventId, providerId, recipientPhone, status, termiiMessageId, messageBody, errorMessage }) {
  ...
  const validEventUuid = (contactEventId && typeof contactEventId === 'string' && UUID_REGEX.test(contactEventId.trim()))
    ? contactEventId.trim()
    : null;
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/artisan_notifications`, {
      ...
      body: JSON.stringify({
        contact_event_id: validEventUuid,
    ...
    // If foreign key constraint failed (e.g. 409 code 23503 because contactEventId was not yet in contact_events),
    // safely retry with contact_event_id: null so the lead notification is never dropped.
    if (res.status === 409 && validEventUuid !== null) {
      const retryRes = await fetch(`${SUPABASE_URL}/rest/v1/artisan_notifications`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
 ...
 */
async function dispatchArtisanLeadAlert({ contactEventId, providerId, locality, intentTag, urgency, explicitPhone, _inject }) {
  try {
    ...
    // Phase 042: Urgency detection & normalized copy construction
    ...
    let urgencyPrefix = '';
    if (urgency && typeof urgency === 'string' && URGENCY_PREFIX_MAP[urgency.toLowerCase().trim()]) {
      urgencyPrefix = URGENCY_PREFIX_MAP[urgency.toLowerCase().trim()] + ' ';
    } else if (trade.startsWith('[URGENT]')) {
      urgencyPrefix = '[URGENT] ';
      trade = trade.replace(/^\[URGENT\]\s*/, '');
    } else if (trade.startsWith('[2-3 DAYS]')) {
      urgencyPrefix = '[2-3 DAYS] ';
      trade = trade.replace(/^\[2-3 DAYS\]\s*/, '');
    } else if (trade.startsWith('[FLEXIBLE]')) {
      urgencyPrefix = '[FLEXIBLE] ';
      trade = trade.replace(/^\[FLEXIBLE\]\s*/, '');
    }

    // Normalize whitespace & control characters to prevent header injection or multi-line exploits
    const cleanTrade = trade.replace(/[\r\n\t\x00-\x1f\x7f]/g, ' ').replace(/\s+/g, ' ').trim().substring(0, 25) || 'Service';
    const cleanLocality = (locality ? String(locality).replace(/<[^>]*>/g, '').replace(/[\r\n\t\x00-\x1f\x7f]/g, ' ').replace(/\s+/g, ' ').trim().substring(0, 25) : 'your area') || 'your area';

    // 3. Construct Privacy-Safe SMS Copy (Strictly Zero Customer PII)
    let smsMessage = '';
    if (urgencyPrefix) {
      smsMessage = `PadiFix Alert: New ${urgencyPrefix}inquiry for ${cleanTrade} in ${cleanLocality}. Open your dashboard or WhatsApp now.`.replace(/\s{2,}/g, ' ');
    } else {
      smsMessage = `PadiFix Alert: You have a new customer inquiry for ${cleanTrade} in ${cleanLocality}. Open your PadiFix dashboard to view the lead.`;
    }

    // Hard deterministic ceiling: final rendered SMS MUST ALWAYS be <= 160 characters
...
/**
 * Canonical reason label mapper for SMS notifications
 */
function getFriendlyRejectionLabel(reasonCode) {
  const map = {
    'blurry_image': 'Blurry photo',
    'expired_document': 'Expired document',
    'name_mismatch': 'Name mismatch',
    'incomplete_document': 'Incomplete document',
    'fraud_suspected': 'Invalid document',
    'other': 'Document review'
  };
  return map[reasonCode] || 'Document review';
}

/**
 * Dispatches a concise, single-segment transactional SMS alert upon verification approval.
 * Template: "PadiFix: Congrats {name}! Your identity is verified. Your Verified Pro badge is now active on your profile."
 * Length: strictly <= 160 chars
 */
async function dispatchVerificationApprovedSms({ phone, providerName = 'Artisan', providerId = null }) {
  try {
    const config = getTermiiConfigurationStatus();
    if (!config.apiConfigured) {
      return { delivered: false, status: 'unconfigured', reason: 'TERMII_NOT_CONFIGURED' };
    }

    const normalizedPhone = normalizeNigerianPhoneForTermii(phone);
    if (!normalizedPhone) {
      return { delivered: false, status: 'invalid_recipient', reason: 'INVALID_PHONE_NUMBER' };
    }

    const cleanName = String(providerName).trim().split(' ')[0] || 'Artisan';
    const smsMessage = `PadiFix: Congrats ${cleanName}! Your identity is verified. Your Verified Pro badge is now active on your profile.`;

    if (!config.senderApproved) {
      return {
        delivered: false,
        status: 'pending_sender_approval',
        simulated: true,
    ...
    const res = await sendTermiiRequest(payload, config.baseUrl, 8000);
    const isOk = res.status === 200 && (res.data?.code === 'ok' || res.data?.message === 'Successfully Sent');
    return {
      delivered: isOk,
      status: isOk ? 'sent' : 'failed',
      messageId: res.data?.message_id || res.data?.id || null,
      length: smsMessage.length
    };
  } catch (err) {
    return { delivered: false, status: 'error', error: err.message };
  }
}

/**
 * Dispatches a concise, single-segment transactional SMS alert upon verification rejection.
 * Template: "PadiFix: Action required on your verification documents ({reason}). Visit padifix.ng/dashboard to resubmit."
 * Length: strictly <= 160 chars
 */
async function dispatchVerificationRejectedSms({ phone, providerName = 'Artisan', reasonCode = 'other', reasonNotes = '', providerId = null }) {
  try {
    const config = getTermiiConfigurationStatus();
    if (!config.apiConfigured) {
      return { delivered: false, status: 'unconfigured', reason: 'TERMII_NOT_CONFIGURED' };
    ...
    const reasonLabel = getFriendlyRejectionLabel(reasonCode);
    const smsMessage = `PadiFix: Action required on your verification documents (${reasonLabel}). Visit padifix.ng/dashboard to resubmit.`;
