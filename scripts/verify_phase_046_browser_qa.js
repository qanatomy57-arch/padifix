/**
 * PADIFIX PHASE 046 — BROWSER QA & REAL-WORLD PILOT READINESS SUITE
 * scripts/verify_phase_046_browser_qa.js
 *
 * Automated Playwright browser verification for Phase 046 pilot readiness:
 * Test 1:  Homepage loads cleanly with PadiFix branding and zero uncaught errors
 ...
 * Test 4:  Artisan registration page loads cleanly with trade selection & location dropdowns
 * Test 5:  Artisan login page functions with password visibility & forgot password routing
 * Test 6:  Forgot password flow displays neutral anti-enumeration protection message
 * Test 7:  Password reset page handles unauthenticated/expired sessions safely
 * Test 8:  Custom 404 error page loads with proper Home and Search navigation routing
...
(async () => {
  console.log('================================================================');
  console.log('  PADIFIX PHASE 046: BROWSER QA & REAL-WORLD PILOT SUITE');
  console.log('================================================================\n');
  ...
  try {
    ...
    if (fs.existsSync(ARTIFACT_DIR)) {
      await page.screenshot({ path: path.join(ARTIFACT_DIR, 'phase_046_homepage_desktop.png') });
      console.log('  📸 Screenshot captured: phase_046_homepage_desktop.png');
    }
    ...
    if (fs.existsSync(ARTIFACT_DIR)) {
      await page.screenshot({ path: path.join(ARTIFACT_DIR, 'phase_046_search_empty_desktop.png') });
      console.log('  📸 Screenshot captured: phase_046_search_empty_desktop.png');
    }
    ...
    // ------------------------------------------------------------------------
    // TEST 4: Artisan Registration Flow Verification
    // ------------------------------------------------------------------------
    ...
    if (fs.existsSync(ARTIFACT_DIR)) {
      await page.screenshot({ path: path.join(ARTIFACT_DIR, 'phase_046_register_desktop.png') });
      console.log('  📸 Screenshot captured: phase_046_register_desktop.png');
    }
    ...
    await page.fill('#forgot-email', 'real.artisan@padifix.ng');
    ...
    if (fs.existsSync(ARTIFACT_DIR)) {
      await mobilePage.screenshot({ path: path.join(ARTIFACT_DIR, 'phase_046_mobile_responsive.png') });
      console.log('  📸 Screenshot captured: phase_046_mobile_responsive.png');
    }
